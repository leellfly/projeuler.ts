...
function getMaximumTotal() {
  ...
  for (let i = 1; i < TRIANGLE.length; i++) {
    const curValue = TRIANGLE[i][curIndex]
    const nextValue = TRIANGLE[i][curIndex + 1] || 0

    maximumTotal += Math.max(curValue, nextValue)

    curIndex = nextValue > curValue ? curIndex + 1 : curIndex;
  }
